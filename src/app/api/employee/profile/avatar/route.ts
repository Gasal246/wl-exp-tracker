...
export async function POST(request: Request) {
  try {
    ...
    const employee = await User.findOne({ _id: sessionResult.session!.user.id, isAdmin: false })
    ...
    if (employee.avatarStoragePath) {
      try {
        await bucket.file(employee.avatarStoragePath).delete({ ignoreNotFound: true });
      } catch {
